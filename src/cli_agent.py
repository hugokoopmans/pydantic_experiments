# cli_agent.py
# simpele commandline interface agent
...
MCP_SERVER_URL = os.getenv("MCP_SERVER_URL", "http://127.0.0.1:8010/mcp")
...
agent = Agent(
    model=model,
    instructions=(
        "Je bent een behulpzame commandline assistent. "
        "Gebruik read_file en write_file voor lokale bestandsacties. "
        "Gebruik MCP tools met prefix 'mcp_' voor server-tools zoals tijd en echo. "
...
async def main() -> None:
    ...
    print(f"MCP server verwacht op: {MCP_SERVER_URL}")
    ...
    async with agent:
        while True:
            ...
            try:
                ...
                logger.exception("❌ Fout tijdens agent run")
                print(f"Fout tijdens run: {exc}")
                print("Controleer of de MCP server draait en MCP_SERVER_URL klopt.")
                print()
                continue
